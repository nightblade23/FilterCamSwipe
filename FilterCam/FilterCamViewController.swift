...
open class FilterCamViewController: UIViewController {
    ...
    public var shouldShowDebugLabels: Bool = false {
        didSet {
            ...
            filterNameLabel.isHidden = !shouldShowDebugLabels
        }
    }
    ...
    private let mySwipeFilters: [[CIFilter]] = [
    ...
    private func setupDebugLabels() {
        ...
        filterNameLabel.leadingAnchor.constraint(equalTo: secLabel.leadingAnchor).isActive = true
        filterNameLabel.topAnchor.constraint(equalTo: secLabel.bottomAnchor).isActive = true
    ...
    @objc private func leftSwipe() {
        filters = mySwipeFilters[getNextFilterIndexForLeftSwipe(currentIndex: currentFilterIndex)]
        currentFilterIndex -= 1
        filterNameLabel.isHidden = false
        filterNameLabel.text = String(filters.description.split(separator: "<")[1].split(separator: ":")[0])
    }
    
    @objc private func rightSwipe() {
        filters = mySwipeFilters[getNextFilterIndexForRightSwipe(currentIndex: currentFilterIndex)]
        currentFilterIndex += 1
        filterNameLabel.isHidden = false
        filterNameLabel.text = String(filters.description.split(separator: "<")[1].split(separator: ":")[0])
    }
    
    private func getNextFilterIndexForLeftSwipe(currentIndex: Int) -> Int {
        if currentIndex == 0 {
            return 0
        } else if currentIndex < 0 {
            currentFilterIndex = mySwipeFilters.count - 1
            return currentFilterIndex
        } else {
            if currentIndex == mySwipeFilters.count {
                currentFilterIndex = mySwipeFilters.count - 2
                return currentFilterIndex
            } else {
                return currentIndex
            }
    ...
    private func getNextFilterIndexForRightSwipe(currentIndex: Int) -> Int {
        if currentIndex == mySwipeFilters.count {
            ...
            if currentIndex < 0 {
                currentFilterIndex = currentIndex * -1
                return currentFilterIndex
            } else {
                return currentIndex
            }
